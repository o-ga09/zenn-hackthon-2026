...
- Genkit Go バックエンド (Cloud Run): `VlogCreatorAgent` 実行、ツール実装、トークン管理、SSEイベント送出
- TiDB: `users`, `subscriptions`, `token_transactions`, `payments`, `vlogs` コレクション
- Cloudflare R2: メディア保存、生成済み動画保存
- Vertex AI: メディア分析、テキスト生成補助
- VOICEVOX: TTS（ずんだもん）
- FFmpeg: 動画合成
- Stripe + Cloud Functions: Checkout 作成、Webhook 処理

シーケンス概略 (Create Vlog):
1. フロントエンドが `POST /api/create-vlog` にファイルと Firebase ID Token で要求
2. バックエンドは認証後、必要トークン数を計算し残高チェック（仮引き/reserve）
3. SSE で `token_check` → `upload_media` → `analyze_media` → `generate_script` → `synthesize_voice` → `create_video` の進捗を送出
4. 処理完了でトークンを確定（reserve → confirm）し、`TokenTransaction` と `VlogOutput` を記録
5. エラー発生時は仮引きをロールバックし、SSEでエラー情報を返す
